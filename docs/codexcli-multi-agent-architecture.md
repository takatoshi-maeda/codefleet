...
## 1. 目的
PM / LeadDeveloper / Developer / QA の複数ロールが、CodexCLI 上で同一リポジトリを安全に扱えるようにする。主な狙いは次の 3 点。
...
```ts
type SystemEvent =
  | { type: "manual.triggered"; actor: "PM" | "QA" | "LeadDeveloper" | "Developer" }
...
- `--include-hidden` で PM/LeadDeveloper のみ確認可能。
